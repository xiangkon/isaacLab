...
class PiperCubeLiftEnvCfg(LiftEnvCfg):
    def __post_init__(self):
        ...
        self.actions.arm_action = mdp.JointPositionActionCfg(
            asset_name="robot", joint_names=["piper_joint.*"], scale=0.5, use_default_offset=True
        )
        self.actions.gripper_action = mdp.BinaryJointPositionActionCfg(
            asset_name="robot",
            joint_names=["piper_finger.*"],
            open_command_expr={"piper_finger.*": 0.04},
            close_command_expr={"piper_finger.*": 0.0},
        )
        # Set the body name for the end effector
        self.commands.object_pose.body_name = "piper_link6"
        ...
        self.scene.ee_frame = FrameTransformerCfg(
            prim_path="{ENV_REGEX_NS}/Robot/piper_link0",
            debug_vis=False,
            visualizer_cfg=marker_cfg,
            target_frames=[
                FrameTransformerCfg.FrameCfg(
                    prim_path="{ENV_REGEX_NS}/Robot/piper_link6",
                    name="end_effector",
                    offset=OffsetCfg(
                        pos=[0.0, 0.0, 0.1034],
                    ),
